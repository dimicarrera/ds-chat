...
io.on("connection", (socket) => {
  ...
  console.info(`new client session: ${id}`);
  ...
  socket.on("send message", (message: Message) => {
    console.info(`message: ${message.author}: ${message.content}`);
  ...
  socket.on("typing...", (username: string) => {
    console.info(`User Typing...: ${username}`);
  ...
  socket.on("stopped typing...", (username: string) => {
    console.info(`User Stopped Typing...: ${username}`);
  ...
  socket.on("disconnect", () => {
    console.info(`user disconnected: ${socket.sessionUsername}`);
